...
let insert (px,py) qtree = 
  ...
  | QNode (bl, tl, br, tr) -> (
    ...
    | true , true -> QNode (impl (x1, y1, xmid, ymid) (px, py) bl, tl, br, tr)
    | true, false -> QNode (bl, impl (x1, ymid, xmid, y2) (px, py) tl, br, tr)
    | false, true -> QNode (bl, tl, impl(xmid, y1, x2, ymid) (px, py) br, tr )
    | false, false ->QNode (bl, tl, br, impl(x1, ymid, xmid, y2) (px, py) tr)
  )
  | Point (px', py') -> if (px', py')=(px, py) then Point (px', py') else 
    impl (x1, y1, x2, y2) (px, py) (impl (x1, y1, x2, y2) (px', py') (QNode (NoPoint, NoPoint, NoPoint, NoPoint)))
